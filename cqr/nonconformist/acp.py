...
class CrossSampler(object):
	...
	"""
	def gen_samples(self, y, n_samples, problem_type):
		if problem_type == 'classification_task':
...
class AggregatedCp(BaseEstimator):
	...
	def predict(self, x, significance=None):
		...
		significance : float or None
			...
			is applicable to classification_task problems only.

		Returns
		-------
		p : numpy array of shape [n_samples, n_classes] or [n_samples, 2]
			For classification_task problems: If significance is ``None``, then p
			...
			For ordinary_regression_task problems: Prediction interval (minimum and maximum
			boundaries) for the set of test patterns.
		"""
		is_regression =\
			self.predictor.__class__.get_problem_type() == 'ordinary_regression_task'
