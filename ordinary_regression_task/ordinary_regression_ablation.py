...
def compute_len_cvg(model, x_train, y_train, x_test, y_test, idx_cal, alpha=0.1, p=1, method='VCP', bias=0):
...
for dataset_name in dataset_names:
    ...
    for i in range(5):
        ...
        for j in range(len(alpha_list)):
            for k in range(len(p_list)):
                ...
                len_PCP, cvg_PCP = compute_len_cvg(model, x_train, y_train, x_test, y_test, idx_cal, alpha_list[j], p_list[k], 'PCP', bias)
                alpha_p_array_VCP[i, j, k] = len_VCP
                alpha_p_array_PCP[i, j, k] = len_PCP

            for k in range(len(bias_list)):
                len_VCP, cvg_VCP = compute_len_cvg(model, x_train, y_train, x_test, y_test, idx_cal, alpha_list[j],
                                                p, 'VCP', bias_list[k])
                len_PCP, cvg_PCP = compute_len_cvg(model, x_train, y_train, x_test, y_test, idx_cal, alpha_list[j],
                                                p, 'PCP', bias_list[k])
                alpha_bias_array_VCP[i, j, k] = len_VCP
                alpha_bias_array_PCP[i, j, k] = len_PCP

    with open(f'ablation_study_result/ablation_study_{dataset_name}_p.csv', mode='w', newline='') as file:
    ...
    print(f'Results saved to ablation_study_result/ablation_study_{dataset_name}_p.csv')

    with open(f'ablation_study_result/ablation_study_{dataset_name}_bias.csv', mode='w', newline='') as file:
        writer = csv.writer(file)
    ...
    print(f'Results saved to ablation_study_result/ablation_study_{dataset_name}_bias.csv')
